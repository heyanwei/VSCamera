...
#ifdef DATE_ROLL_BACK_LOG 
			base::SubsecondPrecision ssPrec(3);
			std::string nowtime = base::utils::DateTime::getDateTime("%Y%M%d", &ssPrec);

			if (currFileSize > 0 && _lastfiletime != nowtime)
			{
				_lastfiletime = nowtime;
				...
				std::string newfile = fname.substr(0, pos) + "_" + nowtime + ".log";
				if (newfile == fname)
				{
					return false;
				}
				...
				fs->open(newfile, std::fstream::out | std::fstream::trunc);
				return true;
			}
			return false;
#else
